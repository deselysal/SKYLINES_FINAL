...
def main():
    ...
    parser.add_argument('--num_timepoints', required=True, type=int,
                        help="Number of time points (models) to use in the skyline")
    parser.add_argument('--t', nargs='+', type=float,
                        help="time points for skyline models, specifying when to switch from model i to model i+1. "
                             "Length should be num_timepoints-1 since model[0] always starts at time 0.")
    ...
    n_timepoints = params.num_timepoints

    # Validate time points
    if params.t is None and n_timepoints > 1:
        raise ValueError("Time points (--t) must be provided when using multiple timepoints")

    if params.t is not None:
        if len(params.t) != n_timepoints - 1:
            raise ValueError(f"Expected {n_timepoints - 1} time points, got {len(params.t)}")
